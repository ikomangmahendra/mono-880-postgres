...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'startDate' | 'endDate'> & {
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      startDate: notification.startDate?.toJSON() ?? null,
      endDate: notification.endDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...
      endDate: restNotification.endDate ? dayjs(restNotification.endDate) : undefined,
    };
  }
